...
(defun my/clang-format-setup ()
  (when (and (locate-dominating-file (buffer-file-name) ".clang-format")
...
#+end_src
*** TODO =cmake-ide=
Add =cmake-ide= so that it's used when a =CMakeLists.txt= and disable
=helm-gtags= when =rtags= is present.
