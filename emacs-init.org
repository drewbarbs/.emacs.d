...
*** Edit code comments in dedicated buffer
The following makes it easy to add doc, e.g. to rust functions
...
(defun my/do-powerline-reset (&rest ignore)
  (powerline-reset))

(defun my/pre-theme-switch (&rest ignore)
...
(use-package darkman
  :ensure t
  :custom
  (darkman-themes '(:light leuven :dark monokai) "Set themes to use during day/night")
  :config
  (advice-add 'darkman--load-theme :before #'my/pre-theme-switch)
  (advice-add 'darkman--load-theme :after #'my/do-powerline-reset)
  (darkman-mode +1))
...
- Jump to a character using =M-g c=
...
(use-package git-link
  ...
  :ensure t
  :config
  (define-advice git-link-github (:around (orig-fun hostname dirname filename branch commit start end) "git-link-force-commit")
    (apply orig-fun hostname dirname filename nil commit start end nil)))
#+end_src
...
(use-package rust-mode
  :ensure t
  :custom
  (rust-format-show-buffer nil "Disable popping up a new window with rustfmt errors")
  ;; rust-analyzer is in the standard tool collection as of 1.64
  ;; https://blog.rust-lang.org/2022/09/22/Rust-1.64.0.html#rust-analyzer-is-now-available-via-rustup
  ;; (lsp-rust-analyzer-server-command '("rustup", "run", "stable", "rust-analyzer") "Use rust-analzyer from rustup")
...
*** DWIM
Have dired suggest the next dired window's directory as the
destination for file copy/move operations
...
(use-package web-mode
  ...
  :mode (("\\.jsx\\'" . web-mode)
         ("\\.tsx\\'" . web-mode))
  :init
  (add-hook 'web-mode-hook #'my/setup-jsx-or-tsx))
...
(use-package typescript-mode
  :ensure t
  :mode "\\.ts\\'"
  :hook (typescript-mode . #'my/setup-ts-mode))
#+end_src
* Misc Modes
** Markdown
#+srcname: name
...
** devicetree
A mode for authoring [[https://www.devicetree.org/][device tree]] source files.
#+begin_src emacs-lisp
