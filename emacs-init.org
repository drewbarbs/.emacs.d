...
** TODO Bringing it home
Here's how I'm going to do the blog:

*** TODO Finish modifying =ox-jakyll= [/]

1. [ ] Change it to =ox-pelican=
2. [ ] Change footnotes to use the markdown export backend
3. [ ] Change codeblock export so that it uses the =.numberLines= and
   =startFrom= attributes when the org source block has =-n= or =+n= [[http://pandoc.org/MANUAL.html#extension-fenced_code_attributes][doc]]

*** TODO Change Pelican config [/]
1. [ ] Use the [[https://github.com/liob/pandoc_reader][pandoc_reader]] pelican markdown reader/get rid of the
   codeblock line numbering stuff.
2. [ ] Change =PANDOC_ARGS= config of =pandoc_reader= to include

   =--bibliography ../refs.bib --filter pandoc-citeproc--csl
   acm-sig-proceedings=
